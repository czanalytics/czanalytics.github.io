...
"""
# access weekly EU fuel prices and process product file

fi = 'latest_prices_raw_data.xlsx'
url = 'https://ec.europa.eu/energy/observatory/reports/'
dir = DAT + 'fuel_price/'

def getFile(f, url, dir):
  !rm {dir}{f} # remove old
  !wget -P {dir} {url}{f}
  !ls -lat {dir}
  import os.path
  status = os.path.exists(dir+f)
  return status

status = getFile(fi, url, dir)
print(status)
...
d = fuelPriceRead(f_prc)
d = fuelPriceEdit(d)
d.info()
d
date_tag = d['date'][0].strftime('%y-%m-%d')
fo = DAT + 'fuel_price/' + 'fuel-price-' + date_tag + '.csv'

d.to_csv(fo, index=False, encoding='utf-8')
f = DAT + 'fuel_price/' + 'fuel-price.csv' # continuity: the latest price has same name
d.to_csv(f, index=False, encoding='utf-8')

!ls {DAT}fuel_price # quality check
!head {DAT}fuel_price/fuel-price.csv
!tail {DAT}fuel_price/fuel-price.csv
"""
